...
- **Update the guide when the test mod teaches you something.** The test mod (`test_mod/`) exists partly to surface gaps and gotchas. Whenever building or iterating on it reveals an engine behaviour, API nuance, encoding quirk, or silent failure mode that is not already covered in the guide, update the relevant `docs/` page in the same session — before committing the mod changes. Do not defer these updates.
